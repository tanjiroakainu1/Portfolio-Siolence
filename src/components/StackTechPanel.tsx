...
export function StackTechPanel({
  ...
  return (
    <div
      className={`rounded-2xl border border-fuchsia-400/22 bg-gradient-to-b from-[#1a0a2e]/88 to-[#120620]/94 p-3 shadow-[0_8px_32px_rgba(0,0,0,0.28),inset_0_1px_0_rgba(255,255,255,0.06)] ring-1 ring-white/[0.05] sm:p-4 ${className}`}
    >
      {eyebrow ? (
        <p className="m-0 mb-2.5 text-[0.62rem] font-semibold uppercase tracking-[0.2em] text-fuchsia-300/80 sm:mb-3 sm:text-[0.65rem]">
          {eyebrow}
        </p>
      ) : null}
      <ul className="m-0 grid list-none gap-2 p-0 sm:gap-2" aria-label={listAriaLabel} role="list">
        {rows.map((row, i) => (
          <li
            ...
            role="listitem"
          >
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-fuchsia-500/28 via-violet-500/14 to-nebula/18 text-fuchsia-100 shadow-[0_4px_14px_rgba(232,121,249,0.15)] ring-1 ring-fuchsia-400/35 sm:h-10 sm:w-10">
